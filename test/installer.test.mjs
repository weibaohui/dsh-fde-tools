...
test('PACK 清单：18 个成员、名字唯一、range 形如 ^x.y.z、user-management 必装', (t) => {
  assert.equal(installer.PACK.length, 18)
  ...
  assert.deepEqual(new Set(ALL_NAMES).size, 18)
...
test('install：全缺 → 一次 pnpm add、逐个写依赖、bundles 全追加、快照落盘', async (t) => {
  ...
  assert.equal(specs.length, 18, '十八个成员一次带齐')
...
test('install：已装成员跳过，不重复进 add', async (t) => {
  ...
  assert.equal(addCall.args.slice(addCall.args.indexOf('add') + 1).length, 17, '只 add 缺的 17 个')
})
...
test('status：安装态 / bundles / 待重启判定', (t) => {
  ...
  assert.equal(st.missingCount, 16)
